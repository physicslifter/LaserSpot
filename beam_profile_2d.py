...
Fits a 2D Gaussian to a beam image (TIFF), finds its center, and extracts
four lineouts through that center -- horizontal, vertical, and the two
45-degree diagonals -- each with its own 1D Gaussian fit for cross-check.

Layout (2x2 dashboard):
    top-left     : raw TIFF with the four lineout directions drawn through
                   the fitted center
    bottom-left  : all four lineouts + their 1D Gaussian fits, overlaid
    top-right    : the 2D fit shown as contours over the (cropped) data
    bottom-right : text report of every fitted parameter

Run with no arguments for a file picker:
    python beam_profile_2d.py

Assumes square pixels (equal pixel pitch in x and y) -- if your camera's
pixel pitch differs between axes, the diagonal lineouts and the reported
widths in pixel units will need a correction factor before comparing them
to the horizontal/vertical widths in physical units.
"""
...
# ==========================================
# File loading (same pattern as view_data.py)
# ==========================================
def load_tiff():
    root = tk.Tk()
    root.withdraw()
...
def gaussian_2d_rotated(coords, amp, x0, y0, sigma_x, sigma_y, theta, offset):
    """Rotated elliptical 2D Gaussian. coords = (x_grid, y_grid), both 2D
    arrays; returns a FLATTENED array, as required by curve_fit."""
    x, y = coords
...
def fit_2d_gaussian(data):
    """Crops a window around the brightest region, fits a rotated 2D
    Gaussian inside it (in LOCAL crop coordinates for numerical stability),
    then converts the fitted center back to GLOBAL image coordinates."""
    background = float(np.median(data))
    data_bs = data.astype(float) - background

    # Coarse peak location and a rough size estimate to size the crop window
...
def sample_lineout(data, x0, y0, angle_deg, half_len, n_samples=400):
    """Samples `data` along a line through (x0, y0) at `angle_deg`
    (0=horizontal, 90=vertical), via sub-pixel interpolation."""
...
def half_width_at_level(s, normalized, level):
    """Rough full-width estimate at a given normalized level, for seeding
    the 1D fit -- not the precision measurement (that's the curve_fit)."""
...
def main():
    ...
    # --- top-left: raw image + lineout directions ---
    ...
    # --- bottom-left: all lineouts + fits, overlaid ---
    ...
    # --- top-right: 2D fit as contours over cropped data ---
